        ...
        const exportGifBtn = document.getElementById('exportGifBtn'); // New GIF export button
        ...
        // Define fixed export resolution for MP4 and GIF
        const EXPORT_WIDTH = 1920; // FHD
        const EXPORT_HEIGHT = 1080; // FHD
        ...
        function startDrawing(e) {
            ...
            activeLayer.strokes.push({
                ...
                color: currentTool === 'pen' ? currentStrokeColor : '#FFFFFF', // Eraser color is white for destination-out
        ...
        function draw(e) {
            ...
            ctx.strokeStyle = currentPath.color; // Eraser still uses a "color" for its path, but composite op makes it erase
        ...
        function drawScene(isExporting = false, exportTimeOverride = null) {
            ...
            layers.forEach(layer => {
                ...
                layer.strokes.forEach(stroke => {
                    ...
                    // Apply erase effect if stroke was made with eraser
                    ...
                    if (stroke.points.length === 1) { 
                        ...
                        targetCtx.arc(p.x, p.y, currentDynamicSize / 2, 0, Math.PI * 2);
                        targetCtx.fillStyle = stroke.color; // For eraser, this will be white, but destination-out makes it erase
                        targetCtx.fill();
                    } else { 
                        ...
                        targetCtx.strokeStyle = stroke.color; // For eraser, this will be white
                        targetCtx.lineWidth = currentDynamicSize;
                        targetCtx.lineCap = 'round'; targetCtx.lineJoin = 'round'; 
                        targetCtx.stroke();
                    }
                    targetCtx.restore(); // Restore from individual stroke transformations and composite op
                });
                targetCtx.restore(); // Restore from layer opacity
            });
        }
        ...
        loadInput.addEventListener('change', (e) => {
            ...
            if (fileExtension === 'drz') {
                ...
                reader.readAsText(file);
            } else if (fileExtension === 'psd' || fileExtension === 'clip') { // Corrected to .clip
        ...
        function parseAndAddSVG(svgString) {
            ...
            // Scale to fit within the EXPORT_WIDTH/EXPORT_HEIGHT for consistency
            const scaleX = EXPORT_WIDTH / vbW;
            const scaleY = EXPORT_HEIGHT / vbH;
            const globalScale = Math.min(scaleX, scaleY) * 0.9; // Fit within 90% of export area

            function transformPoint(x, y) {
                return {
                    x: (x - vbX) * globalScale + (EXPORT_WIDTH - vbW * globalScale) / 2,
                    y: (y - vbY) * globalScale + (EXPORT_HEIGHT - vbH * globalScale) / 2
                };
            }
                ...
                commands.forEach(cmdStr => {
                    ...
                    if (type === 'M') { 
                        ...
                        if (subPathStart && points.length > 0) {
                            ...
                            if(Math.abs(lastPoint.x - firstTransformed.x) > 0.1 || Math.abs(lastPoint.y - firstTransformed.y) > 0.1) { // Check if not already closed
            ...
            svgDoc.querySelectorAll('path, line, rect, circle, ellipse, polyline, polygon').forEach(el => {
                ...
                if (points.length > 0) {
                    activeLayer.strokes.push({
                        points: points, color: strokeColor, size: strokeWidth,
        ...
        exportMp4Btn.addEventListener('click', async () => { 
            ...
            async function compileVideo() {
                ...
                try {
                    ...
                    canvas.width = originalCanvasWidth;
                    canvas.height = originalCanvasHeight;
                    drawScene(false, liveAnimationTimeAtStart); 
        ...
        exportGifBtn.addEventListener('click', async () => {
            ...
            const frameRate = 10; // Lower FPS for GIF
            const durationSeconds = 15;
            const totalFrames = frameRate * durationSeconds;
            const delay = 1000 / frameRate; // Delay between frames in ms
            ...
            const gif = new GIF({
                ...
                quality: 10, // Lower is better, 1-30. 10 is a good default.
                workerScript: 'https://cdnjs.cloudflare.com/ajax/libs/gif.js/0.2.0/gif.worker.js',
                width: EXPORT_WIDTH,
                height: EXPORT_HEIGHT,
                background: '#FFF', // Set background for transparency handling
                transparent: null // Auto-detect, or set to a hex color for transparency
            });
            ...
            function addGifFrame() {
                ...
                exportRenderTime += timeStepPerVideoFrame;
                currentFrame++;
                setTimeout(addGifFrame, 10); // Small delay to allow UI updates
            }

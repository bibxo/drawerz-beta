...
function vectorizeStroke(stroke, forceVectorize = false) {
...
function draw() {
  ...
  for (let s = 0; s < vectorStrokes.length; s++) {
    ...
    if (sketchyIntensity > 0) {
      const seed = strokeTime * 10;
      const maxOffset = sketchyIntensity * 2;
      const randomX = Math.sin(seed) * maxOffset + Math.sin(seed * 0.7) * maxOffset * 0.5;
      const randomY = Math.cos(seed * 1.1) * maxOffset + Math.cos(seed * 0.8) * maxOffset * 0.5;
      ...
      for (let i = 0; i < stroke.points.length; i++) {
        const pt = stroke.points[i];
        const angle = strokeTime * 2 + i * 0.1;
        

        const jitterX = (Math.sin(angle * 3.7 + seed) * 0.5 + Math.sin(seed * 0.8 + i) * 0.5) * sketchyIntensity;
        const jitterY = (Math.cos(angle * 4.2 + seed) * 0.5 + Math.cos(seed * 0.9 + i) * 0.5) * sketchyIntensity;

        const finalX = pt.x + randomX + jitterX;
        const finalY = pt.y + randomY + jitterY;
